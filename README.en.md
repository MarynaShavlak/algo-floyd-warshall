...
- **Theory**
  - [Intuition: what `k` is and why the steps](#intuition)
  - [Core idea (more formally)](#idea)
  - [The update formula](#formula)
- **Example 1 — graph `A–F` (positive weights)**
  - [The example graph](#graph)
  - [Step 1. Initializing the distance matrix](#init)
  - [Steps 2–3. Iterations and updates](#iterations)
  - [Step-by-step walkthrough for each vertex `k`](#walkthrough)
  - [The big picture: evolution of the matrix](#evolution)
  - [Result and path reconstruction](#result)
- **Negative weights**
  - [Negative cycles](#negative-cycles)
  - [Example 2 — a fully negative cycle](#negative-cycle-example)
  - [Example 3 — negative edges (`P, Q, R, S`)](#negative-edges)
- [Step-by-step code execution: code ↔ matrix panels](#code-walkthrough)
- **Wrap-up**
  - [Complexity and comparison](#complexity)
  - [Where it is used](#applications)
  - [Bonus: a cleaner implementation](#clean)
  - [Summary](#summary)
- [License](#license)
...
The directory tree and the separation of responsibilities between modules live in a dedicated file — **[PROJECT_STRUCTURE.en.md](PROJECT_STRUCTURE.en.md)**.
...
The commands for installation, running the examples and the tests, plus a minimal library-usage snippet, are in **[USAGE.en.md](USAGE.en.md)**.

---

<a id="intuition"></a>

## Intuition: what `k` is and why the steps
...
## The update formula (the recurrence)
...
**This is the entire algorithm** — the formula above literally becomes a triple loop. Here is its heart, from [`floyd_warshall_steps`](floyd_warshall/core.py) (the full code — with path reconstruction via `nxt` and the snapshots for the step-by-step pictures — is in [`floyd_warshall/core.py`](floyd_warshall/core.py)):
...
In the teaching version ([`floyd_warshall_steps`](floyd_warshall/core.py)), after each `k` we save a **snapshot** of the matrix and the **set of changed cells** (to highlight them). We also keep a `nxt` matrix to reconstruct the paths themselves: `nxt[i][j]` is the next vertex on the shortest path from `i` to `j`.
...
With `nxt` ready, the full route is built by a simple "follow the pointers until you arrive" loop (this is what [`reconstruct_path`](floyd_warshall/core.py) does):
...
▶️ How [`reconstruct_path`](floyd_warshall/core.py) unfolds the route — step by step, "following the `nxt` pointers" (`A → B → C → D`):
...
## Example 2 — a fully negative cycle (all edges negative)
...
## Example 3 — negative edges (nodes `P, Q, R, S`)
...
## Step-by-step code execution: code ↔ matrix panels
...
Both levels of detail are built from a single step journal (`floyd_warshall/walkthrough.py`, the right panel reuses `draw_matrix`); they are generated by [`examples/05_code_walkthrough.py`](examples/05_code_walkthrough.py).
...
🎬 *MP4 version:* [`code_walk_abcdef_k_C.mp4`](docs/images/en/code_walk_abcdef_k_C.mp4)
...
🎬 *MP4:* [`code_walk_pqrs_k_R.mp4`](docs/images/en/code_walk_pqrs_k_R.mp4)  ·  overview by `k`: [`code_steps_pqrs.png`](docs/images/en/code_steps_pqrs.png)
...
🎬 *MP4:* [`code_walk_xyz_k_Z.mp4`](docs/images/en/code_walk_xyz_k_Z.mp4)  ·  overview by `k`: [`code_steps_xyz.png`](docs/images/en/code_steps_xyz.png)
