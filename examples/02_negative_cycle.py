...
def main() -> None:
    ...
    save_figure(draw_graph(graph_cycle, pos_cycle, labels, highlight_path=[0, 1, 2, 0],
                           title=t("Повністю від'ємний цикл X → Y → Z → X (сума ваг = −3)"), curved=True),
    ...
    print(t("Діагональ матриці D після Флойда–Воршала (вершини X, Y, Z):"),
          [format_value(dist_c[i][i]) for i in range(len(labels))])
    print(t("Є вершина з D[i][i] < 0  →  виявлено від'ємний цикл: {flag}").format(flag=has_negative_cycle(dist_c)))
    print(t("\nЧисла поза діагоналлю алгоритм теж повертає скінченними, але вони НЕ є"))
    print(t("справжніми найкоротшими відстанями — насправді для цих пар відповідь −∞."))
